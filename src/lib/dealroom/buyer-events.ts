...
//
// Pure TS helpers for typed buyer-facing update events. Used by both the
// Convex backend and the frontend. Events are stored in the
// `buyerUpdateEvents` table and surfaced to buyers in the deal room UI.
// Channel-specific rendering (email, push, SMS) is a separate layer that
// consumes these events but does not live here.
//
// NOTE: There is a Convex-side mirror of this file at
// `convex/lib/buyerEvents.ts`. The Convex tsconfig cannot import from
// `../src`, so the two files must be kept in sync manually. Any change
