 ...
 *
 * Pure TS composer shared by backend-facing tests and future UI consumers.
 * The Convex mirror lives in `convex/lib/riskSummary.ts` and must stay in
 * sync with this file.
 *
 * The summary is derived from typed backend state only:
 *   - canonical property facts (flood zone, HOA, roof / wind hardening)
 *   - file-analysis state (contract milestones extracted from documents)
 *
 * Buyers get a buyer-safe projection. Broker/admin callers get the internal
 * projection with review-required file-analysis items and provenance details.
 */
